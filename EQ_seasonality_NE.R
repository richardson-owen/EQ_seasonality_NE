...
# numdays_1901 = length(a)+1 # there are 44925 days from 1901-01-01 to 2023-12-31
# 
# 
...
# records over 80% complete over 1901-2023
# Load and prepare data
complete_record_NE_1901 <- readRDS('C:/Users/richa/OneDrive/Documents/Dartmouth/Hydrology/EQ_2024/complete_record_NE_1901.Rds')
...
GHCNdStationsNE1901 <- GHCNdStationsNE[Station_ID %in% station_list_1901, ]
GHCNdStationsNE1901 <- GHCNdStationsNE1901[, ':='(V7 = NULL, V8 = NULL)]
GHCNdStationsNE1901 <- data.frame(GHCNdStationsNE1901)
...
ggplot() +
  ...
  theme(axis.text = element_blank(), axis.ticks = element_blank(), panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
        panel.background = element_rect(fill = 'Gray85', colour = NA)) +
  ggtitle('NE Grid (1x1 degree), 80% complete stations beginning 1901')
...
# Initialize data table to store top 1% precipitation values for each station
TopPrcp1901 <- data.table()

# Loop through each station
for (i in station_list_1901) {
  station_percent_ <- complete_record_NE_1901[Station_ID == i]
  
  # Filter wet days (> 0.254 mm, data is in 10ths of mm, hence 2.54)
  station_percent_ <- station_percent_[Value > 2.54]
  
  # Calculate the threshold for the top 1% of wet days
  n <- 1
  threshold_value <- quantile(station_percent_$Value, prob = 1 - n/100, na.rm = TRUE)
  station_top_percent <- station_percent_[Value > threshold_value]
  
  # Append results to the main data table
  TopPrcp1901 <- rbind(TopPrcp1901, station_top_percent)
}

# Summing the top precipitation values by year and station
TopPrcp1901[, Year := year(Date)]
sumTopPrcp1901 <- data.table()

years1901 <- 1901:2023

# Loop through each station again
for (i in station_list_1901) {
  station_select <- TopPrcp1901[Station_ID == i]
  ...
  for (j in years1901) {
    ...
    valid_data <- complete_record_NE_1901[Station_ID == i & Year == j]
    
    # Calculate the sum of the top precipitation values for the year
    if (nrow(valid_data) > 0) {
      if (nrow(year_select) > 0) {
        sumTopPrcp <- sum(year_select$Value, na.rm = TRUE)
        year_sum_table <- data.table(Station_ID = i, Year = j, sumTopPrcp = sumTopPrcp)
      } else {
        year_sum_table <- data.table(Station_ID = i, Year = j, sumTopPrcp = 0)
      }
    } else {
      year_sum_table <- data.table(Station_ID = i, Year = j, sumTopPrcp = NA)
    }
    
    sumTopPrcp1901 <- rbind(sumTopPrcp1901, year_sum_table, fill = TRUE)
  }
}
...
sumTopPrcp1901_ORID <- sumTopPrcp1901 %>% left_join(GHCNdStationsNE1901_ORID, by = "Station_ID")

# Finding average precipitation by year, by grid cell
relevantORID1901 <- unique(sumTopPrcp1901_ORID, by = "ORID")
meanAnnGridPrcp1901 <- data.table()
for (i in years1901) {
  year_select <- sumTopPrcp1901_ORID[Year == i]
  grid_mean_table <- data.table()
  for (j in relevantORID1901$ORID) {
  ...
  meanAnnGridPrcp1901 <- rbind(meanAnnGridPrcp1901, grid_mean_table)
}

# Taking the average of all grid averages
year_vs_prcp1901 <- data.table()
for (i in years1901) {
  year_select <- meanAnnGridPrcp1901[Year == i]
  if (nrow(year_select) > 0) {
    year_select[, meanTotalPrcp := mean(meanGridPrcp, na.rm = TRUE)]
    year_select <- unique(year_select, by = "Year")
    year_vs_prcp1901 <- rbind(year_vs_prcp1901, year_select)
  }
}

# Plotting year vs total precipitation
ggplot() + 
  geom_point(data = year_vs_prcp1901, aes(x = Year, y = meanTotalPrcp), color = 'darkblue', size = 3) +
  labs(x = "Year", y = "Total Precipitation (mm)")

# Examining changepoints using changepoint package
chgPcpt <- cpt.mean(year_vs_prcp1901$meanTotalPrcp, penalty = "MBIC", pen.value = 0, method = "AMOC", Q = 5, test.stat = "Normal", class = TRUE, param.estimates = TRUE, minseglen = 1)
...
ggplot() + 
  geom_point(data = year_vs_prcp1901[Year %between% c(1901, 2023)], aes(x = Year, y = meanTotalPrcp / 10), color = 'black', pch = 1, lwd = 1.5) +
  geom_line(data = year_vs_prcp1901[Year %between% c(1901, 2023)], aes(x = Year, y = meanTotalPrcp / 10), color = 'darkgrey', lwd = 0.6) +
  geom_smooth(data = year_vs_prcp1901[Year %between% c(1901, 1996)], aes(x = Year, y = meanTotalPrcp / 10), method = lm, color = 'gold2') +
  geom_smooth(data = year_vs_prcp1901[Year %between% c(1996, 2023)], aes(x = Year, y = meanTotalPrcp / 10), method = lm, color = 'purple') +
  labs(title = "Extreme precipitation trends before and after 1996", x = "Year", y = "Extreme Precipitation (mm)") +
  theme_bw()

# Test for significance and calculate percent change
test_result <- t.test(year_vs_prcp1901[Year < 1996, meanTotalPrcp], year_vs_prcp1901[Year >= 1996, meanTotalPrcp], 
                      alternative = "two.sided", var.equal = FALSE)

# Calculate the percent change
mean_pre_1996 <- test_result$estimate[1]
mean_post_1996 <- test_result$estimate[2]
percent_change <- ((mean_post_1996 - mean_pre_1996) / mean_pre_1996) * 100

# Get the p-value
p_value <- test_result$p.value

# Print the percent change and p-value
print(paste("Percent change from pre-1996 to post-1996:", round(percent_change, 2), "%"))
print(paste("P-value of the change:", p_value))
...
# st_crs(oceans_sf)
oceans_sf = oceans_sf %>% st_transform(3857)
...
# we are really interested in ocean-borne (coastal) prcp, not the st lawrence river
...
for(i in GHCNdStationsNE1901$Station_ID){
  zcalc = data.table()
  zcalc = TopPrcp1901[Station_ID == i]
...
#### Bringing it all together--discharge and pcpt ####

hcdnGageTable = readRDS('hcdnGageTableNE.Rds')

AllPeaks = readRDS("all_peaks.Rds")

# filtering sites, only sites that cover 1950-present (for comparison purposes)
hcdn_more_info = readRDS('hcdn_more_info.RDS')
setDT(hcdn_more_info)
...
# saveRDS(TopPeaks, file = 'TopPeaks.Rds')

## significant changes in flooding via cox-lewis
...
# annotate figure
annotation_text1 <- "Extreme Discharge, Inland North"
...
for (i in hcdn_more_info$site_no) {
  ...
  for (j in years1950) {
    ...
    if (nrow(year_select) > 0) {
      sumTopPeaks = sum(year_select$Flow_norm, na.rm = TRUE)
      year_sum_table = rbind(year_sum_table, data.table(site_no = i, year = j, sumTopPeaks = sumTopPeaks), fill = TRUE)
    } else {
      year_sum_table = rbind(year_sum_table, data.table(site_no = i, year = j, sumTopPeaks = 0), fill = TRUE)
    }
  }
  sumTopQ = rbind(sumTopQ, year_sum_table, fill = TRUE)
}
...
for (i in years1950) {
  ...
  for (j in relevantORID_Q$ORID) {
    grid_select = year_select[ORID == j]
    if (nrow(grid_select) > 0) {
      grid_select[, meanGridPeaks := mean(sumTopPeaks, na.rm = TRUE)]
      grid_select = unique(grid_select, by = "meanGridPeaks")
      grid_mean_table = rbind(grid_mean_table, grid_select, fill = TRUE)
    }
  }
  meanAnnGridPeaks = rbind(meanAnnGridPeaks, grid_mean_table, fill = TRUE)
}
...
for (i in years1950) {
  ...
  if (nrow(year_select) > 0) {
    year_select[, meanTotalPeaks := mean(meanGridPeaks, na.rm = TRUE)]
    year_select = unique(year_select, by = "year")
    year_vs_peakQ = rbind(year_vs_peakQ, year_select, fill = TRUE)
  }
}

# Remove years with NA values before changepoint analysis
year_vs_peakQ_clean <- year_vs_peakQ[!is.na(meanTotalPeaks)]

# Now plotting year vs total extreme Q
ggplot() + 
  geom_point(data = year_vs_peakQ_clean, aes(x = year, y = meanTotalPeaks), color = 'darkblue', size = 3) +
  labs(x = "Year", y = "Total Extreme Flow (cms)")

# Examining changepoints using changepoint package
chgQ <- cpt.mean(year_vs_peakQ_clean$meanTotalPeaks, penalty = "MBIC", pen.value = 0, method = "AMOC", Q = 5, test.stat = "Normal", class = TRUE,
                 param.estimates = TRUE, minseglen = 1)
...
p_time_series = ggplot() + 
  geom_point(data = year_vs_prcp1901[Year %between% c(1950,2023)], aes(x =Year , y = meanTotalPrcp/10), color = 'black', pch = 1, lwd = 1.5) +
  geom_line(data = year_vs_prcp1901[Year %between% c(1950,2023)], aes(x =Year , y = meanTotalPrcp/10), color = 'darkgrey', lwd = 0.6) +
  geom_smooth(data = year_vs_prcp1901[Year %between% c(1950,1996)], aes(x = Year, y = meanTotalPrcp/10), method = lm, color = 'gold2') +
  geom_smooth(data = year_vs_prcp1901[Year %between% c(1996,2023)], aes(x = Year, y = meanTotalPrcp/10), method = lm, color = 'purple') +
